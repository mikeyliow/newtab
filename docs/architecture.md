...
- **Items** collapses to a stat summary by default — two donuts (today: done ÷ done+open `do`; week: done last 7d ÷ done+all open) plus open counts — and expands to the **Today**/**Later** lists on click. Queue items render as the sidebar **Reading list**. There is no privacy/blur mode anymore; the collapsed default covers screen-sharing. Long-term direction is read-mostly: agents and task-app syncs write via MCP, the page is for glancing.
