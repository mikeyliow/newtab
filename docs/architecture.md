...
- **Status line** (widget id `focus`) = icon + one-liner under the greeting, click text to edit, click icon to pick from the curated set (`config.focus` / `config.focus_icon`).
- **Quick links** = labelled pills under the status (`config.quick_links`, seeded with Google Calendar/Mail/Tasks; editable via `PATCH /api/config`).
- **Items** collapses to a stat summary by default — two donuts (today: done ÷ done+open `do`; week: done last 7d ÷ done+all open) plus open counts — and expands to the **Today**/**Later** lists on click. There is no privacy/blur mode anymore; the collapsed default covers screen-sharing. Long-term direction is read-mostly: agents and task-app syncs write via MCP, the page is for glancing.
- **Shortcuts** render as a speed-dial tile grid in the main column (icon over label, first 12 visible, the rest fold into a "+N" popover). Hover a tile for icon-swap/remove; drag to reorder.
- **Library** (widget id `queue`) has its **own table** (`library`: title/url/medium/source/status) — legacy queue-kind items were migrated over on boot. The widget is lean by design: 3 newest visible, everything older behind an "archive · N" expander with internal scroll. Adding items never pushes the page taller. Sports cards sit under it, no heading: F1 with a terracotta countdown chip, Raptors with a green/red W/L chip. The whole page fits one viewport with no scroll at 100% zoom; shortcuts render above Items in the main column.
- **Icons**: shortcuts, quick links, and the status line share a curated lucide set (`src/lib/icons.ts`); shortcuts fall back to the site favicon, then a globe. Icon names are stored in config.
