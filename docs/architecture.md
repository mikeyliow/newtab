...
- **Library** (widget id `queue`) lives in the sidebar with a fixed max height and internal scroll — adding items never pushes the page taller. Sports cards sit under it, no heading: F1 with a terracotta countdown chip, Raptors with a green/red W/L chip. The whole page is designed to fit one viewport with no scroll at 100% zoom.
